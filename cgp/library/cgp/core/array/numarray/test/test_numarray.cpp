...
namespace cgp_test
{
    ...
    void test_numarray()
    {
...

        {
            ...
            assert_cgp_no_msg(a[1].size() == 3);
            assert_cgp_no_msg(is_equal(a[1], {2, 3, 4}));
            assert_cgp_no_msg(a[2].size() == 1);
            assert_cgp_no_msg(is_equal(a[2], {-1}));
            a.push_back(cgp::numarray<int>{7, 8, 9});
            assert_cgp_no_msg(a[3].size() == 3);
            assert_cgp_no_msg(is_equal(a[3], {7, 8, 9}));
        }

        {
            cgp::numarray<int> a;
            assert_cgp_no_msg(cgp::is_equal(type_str(a), "numarray<int>"));
            cgp::numarray<float> b;
            assert_cgp_no_msg(cgp::is_equal(type_str(b), "numarray<float>"));
            cgp::numarray<cgp::numarray<int>> c;
            assert_cgp_no_msg(cgp::is_equal(type_str(c), "numarray<numarray<int>>"));
        }
