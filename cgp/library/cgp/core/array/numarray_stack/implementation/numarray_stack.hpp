...
namespace cgp
{
    ...
    template <typename T, int N>
    std::ostream &operator<<(std::ostream &s, numarray_stack<T, N> const &v);

    /** Direct compiled-checked access to data */
    template <int idx, typename T, int N>
    T const &get(numarray_stack<T, N> const &data);
    template <int idx, typename T, int N>
    T &get(numarray_stack<T, N> &data);

    /** Convert all elements of the buffer to a string.
     * \param buffer: the input buffer
     * \param separator: the separator between each element */
    template <typename T, int N>
    std::string str(numarray_stack<T, N> const &v, std::string const &separator = " ", std::string const &begin = "", std::string const &end = "");
    template <typename T, int N>
    std::string type_str(numarray_stack<T, N> const &);
    template <typename T, int N>
    int size_in_memory(numarray_stack<T, N> const &v);
    template <typename T, int N>
    int size_in_memory(numarray<numarray_stack<T, N>> const &v);
    template <typename T, int N>
    auto const *ptr(numarray_stack<T, N> const &v);
    ...
    bool is_equal(numarray_stack<T1, N1> const &a, numarray_stack<T2, N2> const &b);
    template <typename T, int N>
    bool is_equal(numarray_stack<T, N> const &a, numarray_stack<T, N> const &b);

    /** Math operators
     * Common mathematical operations between buffers, and scalar or element values. */
    template <typename T, int N>
    numarray_stack<T, N> operator-(numarray_stack<T, N> const &a);

    template <typename T, int N>
    numarray_stack<T, N> &operator+=(numarray_stack<T, N> &a, numarray_stack<T, N> const &b);
    template <typename T, int N>
    numarray_stack<T, N> &operator+=(numarray_stack<T, N> &a, T const &b);
    template <typename T, int N>
    numarray_stack<T, N> operator+(numarray_stack<T, N> const &a, numarray_stack<T, N> const &b);
    template <typename T, int N>
    numarray_stack<T, N> operator+(numarray_stack<T, N> const &a, T const &b);
    template <typename T, int N>
    numarray_stack<T, N> operator+(T const &a, numarray_stack<T, N> const &b);
    ...
    numarray_stack<T, N> &operator*=(numarray_stack<T, N> &a, numarray_stack<T, N> const &b);
    template <typename T, int N>
    numarray_stack<T, N> &operator*=(numarray_stack<T, N> &a, float b);
    template <typename T, int N>
    numarray_stack<T, N> operator*(numarray_stack<T, N> const &a, numarray_stack<T, N> const &b);
    template <typename T, int N>
    numarray_stack<T, N> operator*(numarray_stack<T, N> const &a, float b);
    template <typename T, int N>
    numarray_stack<T, N> operator*(float a, numarray_stack<T, N> const &b);
    ...
    //  Condition: ||v||>0
    template <typename T, int N>
    numarray_stack<T, N> normalize(numarray_stack<T, N> const &v);
    // Compute the unit vector without error
    //   if ||v||>0, return v/||v||
    //   otherwise return default_zero_norm vector (without error)
    template <typename T, int N>
    numarray_stack<T, N> normalize(numarray_stack<T, N> const &v, numarray_stack<T, N> const &default_zero_norm);

    // Math operators applied to each component
    template <typename T, int N>
    T max(numarray_stack<T, N> const &v);
    template <typename T, int N>
    T min(numarray_stack<T, N> const &v);
    template <typename T, int N>
    T average(numarray_stack<T, N> const &a);
    template <typename T, int N>
    T sum(numarray_stack<T, N> const &v);
    template <typename T, int N>
    numarray_stack<T, N> abs(numarray_stack<T, N> const &a);
    template <typename T, int N>
    numarray_stack<T, N> clamp(numarray_stack<T, N> const &x, numarray_stack<T, N> const &p_min, numarray_stack<T, N> const &p_max);
...
namespace cgp
{
    ...
    template <typename T, int N>
    int numarray_stack<T, N>::size() const
    {
        return N;
    }

    template <typename T, int N>
    numarray_stack<T, N> &numarray_stack<T, N>::fill(T const &value)
    {
        for (int k = 0; k < N; ++k)
            data[k] = value;
        return *this;
    ...
    template <typename T, int N>
    T const &numarray_stack<T, N>::operator[](int index) const
    {
        check_index_bounds(index, *this);
        return data[index];
    }
    template <typename T, int N>
    T &numarray_stack<T, N>::operator[](int index)
    {
        check_index_bounds(index, *this);
        return data[index];
    }
    ...
    template <typename T, int N>
    typename std::array<T, N>::iterator numarray_stack<T, N>::begin()
    {
        return data.begin();
    }
    template <typename T, int N>
    typename std::array<T, N>::iterator numarray_stack<T, N>::end()
    {
        return data.end();
    }
    template <typename T, int N>
    typename std::array<T, N>::const_iterator numarray_stack<T, N>::begin() const
    {
        return data.begin();
    }
    template <typename T, int N>
    typename std::array<T, N>::const_iterator numarray_stack<T, N>::end() const
    {
        return data.end();
    }
    template <typename T, int N>
    typename std::array<T, N>::const_iterator numarray_stack<T, N>::cbegin() const
    {
        return data.cbegin();
    }
    template <typename T, int N>
    typename std::array<T, N>::const_iterator numarray_stack<T, N>::cend() const
    {
        return data.cend();
    }
    ...
    template <typename T, int N>
    T dot(numarray_stack<T, N> const &a, numarray_stack<T, N> const &b)
    {
        T s = {};
        for (int k = 0; k < N; ++k)
            s += a[k] * b[k];
    ...
    template <typename T, int N>
    T norm(numarray_stack<T, N> const &a)
    {
        using std::sqrt;
        return sqrt(dot(a, a));
    }
