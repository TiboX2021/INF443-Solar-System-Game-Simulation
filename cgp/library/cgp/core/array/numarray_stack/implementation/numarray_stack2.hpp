...
namespace cgp
{
    ...
    template <typename T>
    numarray_stack<T, 2>::numarray_stack()
        : x(T()), y(T())
    {
    }

    template <typename T>
    numarray_stack<T, 2>::numarray_stack(T const &x_arg, T const &y_arg)
        : x(x_arg), y(y_arg)
    {
    }
    ...
    template <typename T>
    int numarray_stack<T, 2>::size() const
    {
        return 2;
    }

    template <typename T>
    numarray_stack<T, 2> &numarray_stack<T, 2>::fill(T const &value)
    {
        x = value;
        y = value;
        return *this;
    ...
    template <typename T>
    T const &numarray_stack<T, 2>::operator[](int index) const
    {
    ...
    template <typename T>
    T &numarray_stack<T, 2>::operator[](int index)
    {
        check_index_bounds(index, *this);
        return (&x)[index];
    }
    ...
    template <typename T>
    T const &numarray_stack<T, 2>::at(int index) const
    {
        check_index_bounds(index, *this);
        return (&x)[index];
    }
    template <typename T>
    T &numarray_stack<T, 2>::at(int index)
    {
        check_index_bounds(index, *this);
        return (&x)[index];
    }
    ...
    template <typename T>
    T *numarray_stack<T, 2>::begin() { return &x; }
    template <typename T>
    T *numarray_stack<T, 2>::end() { return &y + 1; }
    template <typename T>
    T const *numarray_stack<T, 2>::begin() const { return &x; }
    template <typename T>
    T const *numarray_stack<T, 2>::end() const { return &y + 1; }
    template <typename T>
    T const *numarray_stack<T, 2>::cbegin() const { return &x; }
    template <typename T>
    T const *numarray_stack<T, 2>::cend() const { return &y + 1; }

    template <int idx, typename T>
    T const &get(numarray_stack<T, 2> const &data)
    {
        static_assert(idx < 2, "Incorrect element indexing");
        return (&data.x)[idx];
    }
    template <int idx, typename T>
    T &get(numarray_stack<T, 2> &data)
    {
        static_assert(idx < 2, "Incorrect element indexing");
        return (&data.x)[idx];
    }
    ...
    template <typename T>
    std::string type_str(numarray_stack<T, 2> const &)
    {
