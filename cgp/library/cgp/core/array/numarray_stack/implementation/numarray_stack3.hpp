...
namespace cgp
{
    ...
    template <typename T>
    numarray_stack<T, 3>::numarray_stack()
        : x(T()), y(T()), z(T())
    {
    }
    template <typename T>
    numarray_stack<T, 3>::numarray_stack(T const &x_arg, T const &y_arg, T const &z_arg)
        : x(x_arg), y(y_arg), z(z_arg)
    {
    }
    template <typename T>
    numarray_stack<T, 3>::numarray_stack(numarray_stack<T, 2> const &xy, T const &z_arg)
        : x(get<0>(xy)), y(get<1>(xy)), z(z_arg)
    {
    }
    template <typename T>
    numarray_stack<T, 3>::numarray_stack(T const &x_arg, numarray_stack<T, 2> const &yz)
    ...
    template <typename T>
    int numarray_stack<T, 3>::size() const
    {
        return 3;
    }

    template <typename T>
    numarray_stack<T, 3> &numarray_stack<T, 3>::fill(T const &value)
    {
        x = value;
        y = value;
        z = value;
    ...
    template <typename T>
    T const &numarray_stack<T, 3>::operator[](int index) const
    {
    ...
    template <typename T>
    T &numarray_stack<T, 3>::operator[](int index)
    {
        check_index_bounds(index, *this);
        return (&x)[index];
    }
    ...
    template <typename T>
    T const &numarray_stack<T, 3>::at(int index) const
    {
        check_index_bounds(index, *this);
        return (&x)[index];
    }
    template <typename T>
    T &numarray_stack<T, 3>::at(int index)
    {
        check_index_bounds(index, *this);
        return (&x)[index];
    }
    ...
    template <typename T>
    T *numarray_stack<T, 3>::begin() { return &x; }
    template <typename T>
    T *numarray_stack<T, 3>::end() { return &z + 1; }
    template <typename T>
    T const *numarray_stack<T, 3>::begin() const { return &x; }
    template <typename T>
    T const *numarray_stack<T, 3>::end() const { return &z + 1; }
    template <typename T>
    T const *numarray_stack<T, 3>::cbegin() const { return &x; }
    template <typename T>
    T const *numarray_stack<T, 3>::cend() const { return &z + 1; }

    template <typename T>
    numarray_stack<T, 2> numarray_stack<T, 3>::xy() const
    {
        return numarray_stack<T, 2>{x, y};
    }
    template <typename T>
    numarray_stack<T, 2> numarray_stack<T, 3>::yz() const
    {
        return numarray_stack<T, 2>{y, z};
    }
    template <typename T>
    numarray_stack<T, 2> numarray_stack<T, 3>::xz() const
    {
        return numarray_stack<T, 2>{x, z};
    }

    template <int idx, typename T>
    T const &get(numarray_stack<T, 3> const &data)
    {
        static_assert(idx < 3, "Incorrect element indexing");
        return (&data.x)[idx];
    }
    template <int idx, typename T>
    T &get(numarray_stack<T, 3> &data)
    {
        static_assert(idx < 3, "Incorrect element indexing");
        return (&data.x)[idx];
    }
    ...
    template <typename T>
    std::string type_str(numarray_stack<T, 3> const &)
    {
