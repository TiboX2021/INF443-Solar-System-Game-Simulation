...
namespace cgp
{
    ...
    template <typename T>
    numarray_stack<T, 4>::numarray_stack()
        : x(T()), y(T()), z(T()), w(T())
    {
    }
    template <typename T>
    numarray_stack<T, 4>::numarray_stack(T const &x_arg, T const &y_arg, T const &z_arg, T const &w_arg)
        : x(x_arg), y(y_arg), z(z_arg), w(w_arg)
    {
    }
    template <typename T>
    numarray_stack<T, 4>::numarray_stack(numarray_stack<T, 3> const &xyz, T const &w_arg)
        : x(get<0>(xyz)), y(get<1>(xyz)), z(get<2>(xyz)), w(w_arg)
    {
    }
    template <typename T>
    numarray_stack<T, 4>::numarray_stack(T const &x_arg, numarray_stack<T, 3> const &yzw)
        : x(x_arg), y(get<0>(yzw)), z(get<1>(yzw)), w(get<2>(yzw))
    {
    }

    template <typename T>
    numarray_stack<T, 4>::numarray_stack(T const &x_arg, T const &y_arg, numarray_stack<T, 2> const &yz)
        : x(x_arg), y(y_arg), z(get<0>(yz)), w(get<1>(yz))
    {
    }
    template <typename T>
    numarray_stack<T, 4>::numarray_stack(numarray_stack<T, 2> const &xy, T const &z_arg, T const &w_arg)
        : x(get<0>(xy)), y(get<1>(xy)), z(z_arg), w(w_arg)
    {
    }
    template <typename T>
    numarray_stack<T, 4>::numarray_stack(T const &x_arg, numarray_stack<T, 2> const &yz, T const &w_arg)
        : x(x_arg), y(get<0>(yz)), z(get<1>(yz)), w(w_arg)
    {
    }
    template <typename T>
    numarray_stack<T, 4>::numarray_stack(numarray_stack<T, 2> const &xy, numarray_stack<T, 2> const &zw)
        : x(get<0>(xy)), y(get<1>(xy)), z(get<0>(zw)), w(get<1>(zw))
    {
    }

    template <typename T>
    int numarray_stack<T, 4>::size() const
    {
        return 4;
    }

    template <typename T>
    numarray_stack<T, 4> &numarray_stack<T, 4>::fill(T const &value)
    {
        x = value;
        y = value;
        z = value;
    ...
    template <typename T>
    T const &numarray_stack<T, 4>::operator[](int index) const
    {
    ...
    template <typename T>
    T &numarray_stack<T, 4>::operator[](int index)
    {
        check_index_bounds(index, *this);
        return (&x)[index];
    }
    ...
    template <typename T>
    T const &numarray_stack<T, 4>::at(int index) const
    {
        check_index_bounds(index, *this);
        return (&x)[index];
    }
    template <typename T>
    T &numarray_stack<T, 4>::at(int index)
    {
        check_index_bounds(index, *this);
        return (&x)[index];
    }
    ...
    template <typename T>
    T *numarray_stack<T, 4>::begin() { return &x; }
    template <typename T>
    T *numarray_stack<T, 4>::end() { return &w + 1; }
    template <typename T>
    T const *numarray_stack<T, 4>::begin() const { return &x; }
    template <typename T>
    T const *numarray_stack<T, 4>::end() const { return &w + 1; }
    template <typename T>
    T const *numarray_stack<T, 4>::cbegin() const { return &x; }
    template <typename T>
    T const *numarray_stack<T, 4>::cend() const { return &w + 1; }

    template <typename T>
    numarray_stack<T, 3> numarray_stack<T, 4>::xyz() const
    {
        return numarray_stack<T, 3>{x, y, z};
    }
    template <typename T>
    numarray_stack<T, 2> numarray_stack<T, 4>::xy() const
    {
        return numarray_stack<T, 2>{x, y};
    }
    template <typename T>
    numarray_stack<T, 2> numarray_stack<T, 4>::yz() const
    {
        return numarray_stack<T, 2>{y, z};
    }
    template <typename T>
    numarray_stack<T, 2> numarray_stack<T, 4>::xz() const
    {
        return numarray_stack<T, 2>{x, z};
    }

    template <int idx, typename T>
    T const &get(numarray_stack<T, 4> const &data)
    {
        static_assert(idx < 4, "Incorrect element indexing");
        return (&data.x)[idx];
    }
    template <int idx, typename T>
    T &get(numarray_stack<T, 4> &data)
    {
        static_assert(idx < 4, "Incorrect element indexing");
        return (&data.x)[idx];
    }
    ...
    template <typename T>
    std::string type_str(numarray_stack<T, 4> const &)
    {
