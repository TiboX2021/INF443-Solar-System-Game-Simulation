...
#define assert_cgp(X, MSG)                                          \
    {                                                               \
        if ((X) == false)                                           \
        {                                                           \
            cgp::call_error(#X, MSG, __FILE__, __func__, __LINE__); \
        }                                                           \
    }
#define assert_cgp_no_msg(X)                                       \
    {                                                              \
        if ((X) == false)                                          \
        {                                                          \
            cgp::call_error(#X, "", __FILE__, __func__, __LINE__); \
        }                                                          \
    }
#else
#define assert_cgp(X, MSG) \
...
#define error_cgp(MSG)                                          \
    {                                                           \
        cgp::call_error("", MSG, __FILE__, __func__, __LINE__); \
    }
...
#define warning_cgp(MESSAGE_ID, EXTRA)                                      \
    {                                                                       \
        cgp::call_warning(MESSAGE_ID, EXTRA, __FILE__, __func__, __LINE__); \
    }
#else
#define warning_cgp(MESSAGE_ID, EXTRA) \
    {                                  \
