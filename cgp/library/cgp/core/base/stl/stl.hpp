...
namespace cgp
{
    ...
    template <typename T1, typename T2>
    bool contains(std::map<T1, T2> const &m, T1 const &key)
    {
        if (m.find(key) == m.end())
            return false;
        return true;
    ...
    template <typename T>
    bool contains(std::map<std::string, T> const &m, char const *key)
    {
        if (m.find(key) == m.end())
            return false;
        return true;
    ...
    template <typename T1, typename T2>
    std::string str(std::map<T1, T2> const &m)
    {
        std::string str = "{";
    ...
    template <typename T>
    std::string type_str(std::set<T> const &)
    {
        using cgp::type_str;
