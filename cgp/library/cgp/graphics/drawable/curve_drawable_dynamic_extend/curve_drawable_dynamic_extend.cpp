...
namespace cgp
{
    ...
    curve_drawable_dynamic_extend &curve_drawable_dynamic_extend::push_back(cgp::vec3 const &p)
    {
        if (int(vbo_position.size) <= int(N_valid_points + 1)) // Need resizing - the VBO is too small
        {
            ...
            glBindBuffer(GL_COPY_READ_BUFFER, vbo_position.id);
            opengl_check;
            glBindBuffer(GL_COPY_WRITE_BUFFER, temp_vbo.id);
            opengl_check;
            glCopyBufferSubData(GL_COPY_READ_BUFFER, GL_COPY_WRITE_BUFFER, 0, 0, size_to_copy);
            opengl_check;
        ...
        glBindBuffer(GL_ARRAY_BUFFER, vbo_position.id);
        opengl_check;
        glBufferSubData(GL_ARRAY_BUFFER, N_valid_points * 3 * sizeof(float), 3 * sizeof(float), ptr(p));
        opengl_check;
        N_valid_points++;
