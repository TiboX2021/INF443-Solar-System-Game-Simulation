...
namespace cgp
{
    ...
    void draw(skybox_drawable const &skybox, SCENE_ENVIRONMENT const &environment)
    {
        ...
        opengl_check;
        skybox.texture.bind();
        opengl_uniform(skybox.shader, "image_skybox", 0);
        opengl_check;

        // Call draw function
        glBindVertexArray(skybox.vao);
        opengl_check;
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, skybox.ebo_connectivity.id);
        opengl_check;
        glDrawElements(GL_TRIANGLES, GLsizei(skybox.ebo_connectivity.size * 3), GL_UNSIGNED_INT, nullptr);
        opengl_check;
