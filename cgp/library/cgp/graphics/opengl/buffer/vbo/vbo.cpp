...
namespace cgp
{
    ...
    static GLuint opengl_buffer_data_initialize_generic(numarray<numarray_stack<float, N>> const &data, GLuint buffer_type, GLenum draw_type)
    {
        ...
        opengl_check;
        glBindBuffer(buffer_type, vbo_index);
        opengl_check;
        glBufferData(buffer_type, GLsizeiptr(size_in_memory(data)), ptr(data), draw_type);
        opengl_check;
    ...
    void opengl_vbo_structure::update(numarray<vec2> const &data, int size_elements_update)
    {
        ...
        {
            glBufferSubData(GL_ARRAY_BUFFER, 0, size_in_memory(data), ptr(data));
            opengl_check;
        }
        else
        {
            glBufferSubData(GL_ARRAY_BUFFER, 0, 2 * sizeof(float) * size_elements_update, ptr(data));
            opengl_check;
        }
    }
    void opengl_vbo_structure::update(numarray<vec3> const &data, int size_elements_update)
    {
        ...
        {
            glBufferSubData(GL_ARRAY_BUFFER, 0, 3 * sizeof(float) * size_elements_update, ptr(data));
            opengl_check;
        }
    }
    void opengl_vbo_structure::update(numarray<vec4> const &data, int size_elements_update)
    {
        ...
        {
            glBufferSubData(GL_ARRAY_BUFFER, 0, 4 * sizeof(float) * size_elements_update, ptr(data));
            opengl_check;
        }
    }
    ...
    void opengl_set_vao_location(opengl_vbo_structure const &vbo, GLuint location_index)
    {
        ...
        opengl_check
            glVertexAttribPointer(location_index, vbo.details.size_element, vbo.details.type_element, GL_FALSE, 0, nullptr);
        opengl_check
            vbo.unbind();
    }
