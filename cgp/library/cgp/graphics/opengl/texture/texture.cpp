...
namespace cgp
{
                                                      ...
                                                      bool is_mipmap, GLint texture_mag_filter, GLint texture_min_filter)
    {
        ...
        glTexImage2D(texture_type, 0, format, width, height, 0, gl_format, data_type, data);
        opengl_check;
    ...
    GLuint opengl_load_texture_image(image_structure const &im, GLint wrap_s, GLint wrap_t)
    {
        ...
        {
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, im.width, im.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, ptr(im.data));
            opengl_check;
        }
        if (im.color_type == image_color_type::rgb)
        {
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, im.width, im.height, 0, GL_RGB, GL_UNSIGNED_BYTE, ptr(im.data));
            opengl_check;
        }
        glGenerateMipmap(GL_TEXTURE_2D);
        opengl_check;
    ...
    GLuint opengl_load_texture_image(grid_2D<vec3> const &im, GLint wrap_s, GLint wrap_t)
    {
        ...
        // Send texture on GPU
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, GLsizei(im.dimension.x), GLsizei(im.dimension.y), 0, GL_RGB, GL_FLOAT, ptr(im.data));
        opengl_check;
        glGenerateMipmap(GL_TEXTURE_2D);
