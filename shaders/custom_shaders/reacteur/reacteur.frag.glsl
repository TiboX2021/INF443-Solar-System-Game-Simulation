...
void main()
{
    ...
    // On conserve la couleur de la texture, mais on la combine avec la couleur du reacteur :
    vec3 color_shading_texture = (Ka + Kd * diffuse_component) * color_object + Ks * specular_component * vec3(1.0, 1.0, 1.0);

    vec3 color_shading = (1-rapidite) * color_shading_texture + rapidite * vec3(0.5, 0.7, 1);
