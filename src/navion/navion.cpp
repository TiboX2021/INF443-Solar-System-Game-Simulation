...
void Navion::set_orientation(const rotation_transform &orientation)
{
    hierarchie["Corps"].transform_local.rotation = orientation;
}

void Navion::update_mesh_hierarchy()
{
    hierarchie.update_local_to_global_coordinates();
}
...
void Navion::create_vaisseau_vador(float const &scale)
{
    ...
    // Pour le cocpit : une demi-sph�re, 24 tuyaux
...
mesh Navion::transversale_vador(float const &scale)
{
    ...
    truc.position[0] = {0, 0, 0}; // en vrai, inutile mais je me suis chibr� dans la num�rotation
...
void Navion::create_star_destroyer(float const &scale)
{
    ...
    // Pour les flammes des reacteurs, on utilise la classe reacteur : il suffit d'ajouter les positions et directions � la liste
...
mesh Navion::corps_destroyer(float const &scale)
{
    ...
    // la connectivit� entre les triangles :
    // l'arriere:
...
mesh Navion::tour_destroyer(float const &scale)
{
    ...
    //  base de la tour:     cot�:     face avant:
    //   2---------3           4       4----5
    //   |         |         / |       |    |
    //   |         |        /  |       |    |
    //   0---------1       2---0       0----1
...
mesh Navion::poste_de_commande_destroyer(float const &scale)
{
    ...
    //    8------____         ____------4
    //               ----6----
    // et idem de l'autre c�t�
    //
    // Probl�me : la texture s'applique de mani�re bizarre
    // Solution : doubler les points pour avoir une texture coh�rente (Probl�mes de shaders apr�s ?)
    ...
    //*******************************************************************//
    // On d�finit ensuite les points qui serviront � former les faces avant et arri�re :
    //*******************************************************************//
    ...
    //************************************************//
    //    d'abord on d�finit les point qui serviront pour les c�t�s, avec les coordonn�es de texture qui conviennent //
    //************************************************//
    ...
    // On d�finit ensuite la connectivit�
